...
async function checkAndIncrementLoggedInUsage(userId) {
  // ===== DAILY CAP CHECK (Redis) =====
  // Free logged-in users are capped at FREE_LOGGED_IN_DAILY_LIMIT messages per
  // UTC day in addition to the monthly cap. Daily counter lives in Redis
  // because it's ephemeral and we don't need persistence past 36h.
  // We check the daily counter BEFORE the monthly counter so blocked-on-daily
  // requests don't consume monthly allowance unnecessarily.
  const day = getCurrentDay();
  const dailyKey = `daily:${userId}:${day}`;
  try {
    const dailyCount = await redis.incr(dailyKey);
    if (dailyCount === 1) {
      // First message of the day — set TTL so the key expires automatically
      await redis.expire(dailyKey, 36 * 60 * 60);
    }
    if (dailyCount > FREE_LOGGED_IN_DAILY_LIMIT) {
      // Daily cap hit. Roll back the increment so it doesn't keep climbing
      // past the limit forever (matters if user retries many times after hitting cap).
      await redis.decr(dailyKey);
      return {
        allowed: false,
        used: FREE_LOGGED_IN_DAILY_LIMIT,
        remaining: 0,
        reason: 'daily',
      };
    }
  } catch (err) {
    console.error('Daily cap Redis error:', err);
    // Fail open on Redis errors — Supabase monthly check still gates abuse
  }

  // ===== MONTHLY CAP CHECK (Supabase) =====
